...
if (isset($_POST['ssn_search'])) {

    $ssn_search = $_POST['ssn_search'];

    $sql = "SELECT * FROM person WHERE ssn = '$ssn_search';";
    ...
    if (count($myarr) > 0) {
        ...
        echo "<script>alert('Person Successfully Found.')</script>";
    } else {
        echo "<script>alert('Failed to Find.')</script>";
    }
}

if (isset($_POST['pid_search'])) {

    $pid_search = $_POST['pid_search'];

    $sql = "SELECT * FROM patient_receive p1, patient_receive2 p2, patient_receive3 p3 WHERE p1.pid=p2.pid AND p1.pid=p3.pid AND p2.pid=p3.pid AND p1.pid=$pid_search";
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="text-center col-md-6 offset-md-3 mt-4">
                <h1>PERSON SEARCH RESULT TABLE</h1>
            </div>
            ...
            <div class="offset-md-1 mt-4" title="patientSearch">
                <form method="post" action="patientTable.php">
                    <input class="cusBut" id="submit" type="submit" value="RETURN TO PATIENT TABLE"><br>
                </form>
