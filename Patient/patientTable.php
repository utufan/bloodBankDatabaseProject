...
$check = mysqli_query($link, "SELECT * FROM patient_receive p1, patient_receive2 p2, patient_receive3 p3 WHERE p1.pid=p2.pid AND p1.pid=p3.pid AND p2.pid=p3.pid");
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="text-center col-md-6 offset-md-2 mt-4">
                <h1 class="textColor">PATIENT TABLE</h1>
            </div>
            <table class="table textColor table-hover col-md-10 offset-md-1">
                ...
                <tbody>
                    ...
                    for ($i = 0; $i < $row_number; $i++) {
                        $ssn = $myarr[$i]['ssn'];
                        $pid = $myarr[$i]['pid'];
                        $bloodType = $myarr[$i]['bloodType'];
                        $illness = $myarr[$i]['illness'];
                        $hid = $myarr[$i]['hid'];
                        echo "<tr>";
                        echo "<td>" . $ssn .  "</td>";
                        echo "<td>" . $pid . "</td>";
            ...
            <div class="offset-md-1 mt-4">
                <div>
                    <h4 class="textColor">To Add a Patient into our Database</h4>
                </div>
                <form name="add" id="add" method="post" action="patientAdd.php" onsubmit="return validateForm('add');">
                    <input class="inputField" id="ssn" type="text" name="ssn" placeholder="Social Security Number">
                    <input class="inputField" id="pid" type="text" name="pid" placeholder="Patient ID">
                    <input class="inputField" id="bloodType" type="text" name="bloodType" placeholder="Bloodtype">
                    <input class="inputField" id="illness" type="text" name="illness" placeholder="Illness">
                ...
                <div>
                    <h4 class="textColor">To Search with Patient ID in our Database</h4>
                </div>
                <form name="search" id="search" method="post" action="patientSearch.php" onsubmit="return validateForm('search');">
                    <input class="inputField" id="pid_search" type="text" name="pid_search" placeholder="Patient ID">
                    <input class="cusBut" id="submit" type="submit" value="SEARCH"><br>
                </form>
                <div>
                    <h4 class="textColor">To Delete a Patient from our Database</h4>
                </div>
                <form name="delete" id="delete" method="post" action="patientDelete.php" onsubmit="return validateForm('delete');">
                    <input class="inputField" id="pid_delete" type="text" name="pid_delete" placeholder="Patient ID">
                    <input class="cusBut" id="submit" type="submit" value="DELETE"><br>
                </form>
            </div>
        </div>
    </div>
