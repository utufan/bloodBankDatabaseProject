...
if (isset($_POST['hid_search'])) {

    $hid_search = $_POST['hid_search'];

    $sql = "SELECT * FROM hospital WHERE hid = '$hid_search';";
    ...
    if (count($myarr) > 0) {
        ...
        echo "<script>alert('Hospital ID Successfully Found.')</script>";
    } else {
        echo "<script>alert('Failed to Find.')</script>";
    }
}

if (isset($_POST['hname_search'])) {

    $hname_search = $_POST['hname_search'];

    $sql = "SELECT * FROM hospital WHERE hname = '$hname_search';";
    ...
    if (count($myarr) > 0) {
        ...
        echo "<script>alert('Hospital Name Successfully Found.')</script>";
    } else {
        echo "<script>alert('Failed to Find.')</script>";
    }
}
    ...
    <div class="container-fluid" style="height:87%">
        <div class="row">
            <div class="text-center col-md-6 offset-md-3 mt-4">
                <h1>HOSPITAL SEARCH RESULT TABLE</h1>
            </div>
            <table class="table table-hover col-md-10 offset-md-1 mt-4">
                <thead>
                    <tr>
                        <th>Hospital ID</th>
                        <th>Hospital Name</th>
            ...
            <div class="offset-md-1 mt-4" title="hospitalSearch">
                <form method="post" action="hospitalTableClient.php">
                    <input class="cusBut" id="submit" type="submit" value="RETURN TO HOSPITAL PAGE"><br>
                </form>
