<html>

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
    <title>Person Table</title>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css"> <!--  using boost library    -->
    <link rel="icon" href="data:image/png;base64,">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
</head>

<script>
    function validateForm(formId) {
        var inputs, index;
        var form = document.getElementById(formId);
        inputs = form.getElementsByTagName('input');
        for (index = 0; index < inputs.length; ++index) {
            // deal with inputs[index] element.
            if (inputs[index].value == null || inputs[index].value == "") {
                alert("Field is empty");
                return false;
            }
        }
    }
</script>

<?php
session_start();
if ($_SESSION['status'] != "ACTIVE") {
    header("Location: homepage.php");
}

error_reporting(E_ALL ^ E_DEPRECATED);

$link = mysqli_connect("localhost", "root", "", "proje");

// Check connection
if ($link === false) {
    die("ERROR: Could not connect. " . mysqli_connect_error());
}

$check = mysqli_query($link, "SELECT * FROM person");

if (!$check) { // add this check.
    die('Invalid query: ' . mysql_error());
}

$myarr = array();

while ($row = mysqli_fetch_array($check)) {
    array_push($myarr, $row);
}

ini_set('display_errors', 1);
ini_set('display_startup_errors', 1);
error_reporting(E_ALL);

?>

<style>
    .example {

        background-image: url(bbBackground.jpg);
        background-position: center;
        background-repeat: no-repeat;
        background-size: cover;

    }

    .cusBut {
        background-color: transparent;
        border: none;
        color: white;

    }

    .cusBut:hover {
        background-color: transparent;
        border: none;
        color: white;
        box-shadow: 0 12px 16px 0 rgba(0, 0, 0, 0.24), 0 17px 50px 0 rgba(0, 0, 0, 0.19);
    }

    .homeBut {
        background-color: transparent;
        border: none;
        color: white;
        font-size: 50px;
    }

    .textColor {

        color: white;

    }
</style>

<body class="example">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-1 mt-4" title="Patates">
                <button class="homeBut" onclick="window.location='admin_page.php'"><i class="fa fa-home"></i></button>
            </div>
            <div class="text-center col-md-6 offset-md-2 mt-4">
                <h1 class="textColor">PERSON TABLE</h1>
            </div>
            <table class="table textColor table-hover col-md-10 offset-md-1">
                <thead>
                    <tr style="background-color:transparent">
                        <th>Social Security Number</th>
                        <th>Name</th>
                        <th>Age</th>
                        <th>Sex</th>
                        <th>Phone No</th>
                        <th>Address</th>
                    </tr>
                </thead>
                <tbody>

                    <?php
                    $row_number = count($myarr);

                    for ($i = 0; $i < $row_number; $i++) {
                        $ssn = $myarr[$i]['ssn'];
                        $name = $myarr[$i]['name'];
                        $age = $myarr[$i]['age'];
                        $sex = $myarr[$i]['sex'];
                        $phno = $myarr[$i]['phno'];
                        $paddress = $myarr[$i]['paddress'];
                        echo "<tr>";
                        echo "<td>" . $ssn .  "</td>";
                        echo "<td>" . $name .  "</td>";
                        echo "<td>" . $age . "</td>";
                        echo "<td>" . $sex .  "</td>";
                        echo "<td>" . $phno .  "</td>";
                        echo "<td>" . $paddress .  "</td>";
                        echo "</tr>";
                    }

                    ?>
                </tbody>

            </table>

            <div class="offset-md-1 mt-4">
                <div>
                    <h4 class="textColor">To Add a Person into our Database</h4>
                </div>
                <form name="add" id="add" method="post" action="personAdd.php" onsubmit="return validateForm('add');">
                    <input class="inputField" id="ssn" type="text" name="ssn" placeholder="Social Security Number">
                    <input class="inputField" id="name" type="text" name="name" placeholder="Name">
                    <input class="inputField" id="age" type="text" name="age" placeholder="Age">
                    <input class="inputField" id="sex" type="text" name="sex" placeholder="Sex(M or F)">
                    <input class="inputField" id="phno" type="text" name="phno" placeholder="Phone No">
                    <input class="inputField" id="paddress" type="text" name="paddress" placeholder="Address">
                    <input class="cusBut" id="submit" type="submit" value="INSERT"><br>
                </form>
                <div>
                    <h4 class="textColor">To Search with Social Security Number in our Database</h4>
                </div>
                <form name="search" id="search" method="post" action="personSearch.php" onsubmit="return validateForm('search');">
                    <input class="inputField" id="ssn_search" type="text" name="ssn_search" placeholder="Social Security Number">
                    <input class="cusBut" id="submit" type="submit" value="SEARCH"><br>
                </form>
                <div>
                    <h4 class="textColor">To Search with Name in our Database</h4>
                </div>
                <form name="search2" id="search2" method="post" action="personSearch.php" onsubmit="return validateForm('search2');">
                    <input class="inputField" id="name_search" type="text" name="name_search" placeholder="Name">
                    <input class="cusBut" id="submit" type="submit" value="SEARCH"><br>
                </form>
            </div>
        </div>
    </div>
</body>

</html>