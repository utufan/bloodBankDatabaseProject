<html lang="en">

<script>
    //hiding button not working
    session_start();
    if ($_SESSION['status'] == "ACTIVE") {

        document.getElementById('login').style.display = "none";
        document.getElementById('logout').style.display = "inline";

    } else if ($_SESSION['status'] == "DEACTIVE") {

        document.getElementById('login').style.display = "inline";
        document.getElementById('logout').style.display = "none";

    }
</script>

<?php
$link = mysqli_connect("localhost", "root", "", "proje");

// Check connection
if ($link === false) {
    die("ERROR: Could not connect. " . mysqli_connect_error());
}

$check = mysqli_query($link, "SELECT * FROM  demands");

if (!$check) { // add this check.
    die('Invalid query: ' . mysql_error());
}

$myarr = array();
$counterA1 = 0;
$counterA2 = 0;
$counterB1 = 0;
$counterB2 = 0;
$counterAB1 = 0;
$counterAB2 = 0;
$counter01 = 0;
$counter02 = 0;

while ($row = mysqli_fetch_array($check)) {
    array_push($myarr, $row);
}

$row_number = count($myarr);

for ($i = 0; $i < $row_number; $i++) {
    $blood = $myarr[$i]['blood'];
    if ($blood == "A+") {
        $counterA1 = $counterA1 + 1;
    }
    if ($blood == "A-") {
        $counterA2 = $counterA2 + 1;
    }
    if ($blood == "B+") {
        $counterB1 = $counterB1 + 1;
    }
    if ($blood == "B-") {
        $counterB2 = $counterB2 + 1;
    }

    if ($blood == "AB+") {
        $counterAB1 = $counterAB1 + 1;
    }
    if ($blood == "AB-") {
        $counterAB2 = $counterAB2 + 1;
    }

    if ($blood == "0+") {
        $counter01 = $counter01 + 1;
    }
    if ($blood == "0-") {
        $counter02 = $counter02 + 1;
    }
}

?>


<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
    <link rel="icon" href="data:image/png;base64,">

    <style>
        .customBtn {

            margin: 0px 10px 10px 0px;
            height: 80px;
            width: 550px;
            color: white;
            background-color: darkgray;

        }

        .topLeftImage {

            background-size: 120px;
            height: 120px;
            width: 120px;
            margin-right: 50px;

        }

        .loginBut {
            text-decoration: none;
            color: #E52C34;
            display: inline;

        }

        .logoutBut {
            text-decoration: none;
            color: #E52C34;
        }
    </style>

    <title>Patates</title>
</head>

<body>
    <div class="container-fluid" style="height:20%">
        <div class="row">
            <nav style="background-color:white" class="navbar navbar-expand-lg navbar-light bg-white col-md-11 offset-md-1">
                <a href="homepage.php" id="homepage" onclick="document.location=this.id+'.php';return false;">
                    <img src="handIcon.jpg" class="topLeftImage" alt="Blood Donation" title="Patates">
                </a>
                <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarText" aria-controls="navbarText" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon"></span>
                </button>
                <div class="collapse navbar-collapse" id="navbarText">
                    <ul class="navbar-nav mr-auto">
                        <li class="nav-item">
                            <a class="nav-link" href="hospitalTableClient.php" style="font-size: 25px; margin-right: 25px">Hospital <span class="sr-only">(current)</span></a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="information_page.php" style="font-size: 25px; margin-right: 25px">Information</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="staff.php" style="font-size: 25px; margin-right: 25px">Our Staff</a>
                        </li>
                        <li>
                            <a class="nav-link" href="admin_page.php" style="font-size: 25px;">Admin Page</a>
                        </li>
                    </ul>

                    <a id="login" class="col-md-1 offset-md-3 loginBut" href="login_pg.php">
                        Log In
                    </a>
                    <a id="logout" class="col-md-1 offset-md-1 logoutBut" href="logout.php">
                        Log Out
                    </a>
                </div>
            </nav>
        </div>
    </div>
    <div class="container-fluid">
        <div class="row col-md-11 offset-md-1" style="height:67%">
            <div class="col-md-4 offset-md-1" style="margin-top: 30px">

                <h2 style="width:350px; font-size: 40px; margin-top: 50px;"> Save Lives. <br>Donate
                    Blood. <img id="blood" style="width:32px ;" src="bloodDrop.jpg" alt="Alternate Text"></h2>
                <h4>Currently we need: <br></h4>

                <div style="text-align:left;width:65%">
                    <span style="float:left">

                        <?php
                        echo 50 * $counterA1, " Litres of A+ <br>";
                        echo 50 * $counterA2, " Litres of A- <br>";
                        echo 50 * $counterB1, " Litres of B+ <br>";
                        echo 50 * $counterB2, " Litres of B- <br>";
                        ?>
                    </span>
                </div>

                <div style="text-align:right;width:65%">
                    <span style="float:right">
                        <?php
                        echo 50 * $counterAB1, " Litres of AB+ <br>";
                        echo 50 * $counterAB2, " Litres of AB-   <br>";
                        echo 50 * $counter01, " Litres of 0+ <br>";
                        echo 50 * $counter02, " Litres of 0- <br>";
                        ?>
                    </span>
                </div>
            </div>
            <div class="col-md-4 offset-md-1" style="margin-top: 30px">
                <h2 style="width:600px; font-size: 40px; margin-top: 50px;"> Want to Become a Donor? <img id="syringe" style="width:32px ;" src="syringe.jpg" alt="Alternate Text"> <br> Register to our
                    Database.</h2>
                <button id="registerBut" class="btn btn-danger offset-md-3 mt-4" style="height:70px; width: 200px; font-size: 20px" onclick="window.location='personRegister.php'">REGISTER</button>

            </div>
        </div>
        <div class="row" style="background-color:#E52C34; height:13%">


        </div>
    </div>
</body>

</html>